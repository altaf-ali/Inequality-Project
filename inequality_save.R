...
#nightlight_obj <- raster::resample(masked_obj(nightlight_data[[year - 1992 + 1]], spatial_data), population_obj)

grids <- rasterToPolygons(population_obj)
groups_obj <- population_groups[population_groups$gwid == country_cown,]

#grid_table <- build_grid_table(population_obj, groups_obj, nightlight_obj)

#nightlight_obj <- masked_obj(nightlight_data[[1]], spatial_data)
#nightlight_obj <- raster::resample(masked_obj(n, spatial_data), population_obj)

#grid_table <- build_grid_table(population_obj, groups_obj, nightlight_obj)

#xx = nightlight_data[1:2]
...
grid_table <- dplyr::bind_rows(lapply(nightlight_data, function(n) { 
  ...
  message(paste("building grid table for", nightlight_year))
